...
num_experiments = 50
swap = False
folder_path = "../../results/test/age_data_swap_false"
...
small_X_noise, small_bounds = gaussian_noise(small_X, bounds, scale=scale_for_gaussian)
large_X_noise, large_bounds = gaussian_noise(large_X, bounds, scale=scale_for_gaussian)
...
for i, m in enumerate(m_list):
    for j in range(num_experiments):
        ...
        start = time.time()
        estimates = dp_aq(large_X_noise, quantiles[i], bounds, eps, swap=swap)
        ...
        start = time.time()
        estimates = dp_aq(large_X_noise, quantiles[i], bounds, rho, swap=swap, cdp=True)
        ...
        # Run Kaplan et al. algorithm
        start = time.time()
        estimates = dp_aq(small_X_noise, quantiles[i], bounds, eps, swap=swap)
        ...
        # Run Kaplan et al. algorithm with approximate DP
        start = time.time()
        estimates = dp_aq(small_X_noise, quantiles[i], bounds, rho, swap=swap, cdp=True)
