...
m_list = range(10, 100, 10)  # number of quantiles
...
for i, m in enumerate(m_list):
    for j in range(num_experiments):
        ...
        # Run our mechanism
        our_estimates, flag_random = slice_quantiles(X, q_list=quantiles[i], eps=eps, bound=bounds, split=0.3,
                                                     seed=seed, swap=True)
...
plt.title("Guassian Dataset - Fraction of Random Values Returned", fontsize=16)
plt.xlabel("Number of Quantiles (m)", fontsize=12)
